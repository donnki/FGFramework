...
skill.action = function(p)
    ...
    for i=1,4 do
        ...
        while not WaitForSeconds(p, 0.5) do coroutine.yield() end
        print("TODO: 对每一个单位造成50点伤害")
        while not WaitForSeconds(p, 1.5) do coroutine.yield() end
         
    end
    while not WaitForSeconds(p, 1) do coroutine.yield() end
